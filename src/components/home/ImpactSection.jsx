      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {CARDS.map(({ icon: Icon, color, ring, title, body, cta, page, ctaClass }) => (
            <div key={title} className={`rounded-2xl p-8 ring-1 ${ring} bg-white backdrop-blur-md shadow-sm hover:ring-opacity-60 transition-all hover:bg-gray-50`}>
              <div className={`w-14 h-14 rounded-2xl ${color} flex items-center justify-center mb-5 text-white`}>
                <Icon className="w-7 h-7" />
              </div>
              <h3 className="text-xl font-bold text-gray-900 mb-3">{title}</h3>
              <p className="text-gray-700 text-sm leading-relaxed mb-6">{body}</p>
              <Link
